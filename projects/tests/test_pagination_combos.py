...
class PaginateCombinedWithFilterTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.male_mice = MouseFactory.create_batch(18, project=cls.project, sex="M")
        cls.female_mice = MouseFactory.create_batch(18, project=cls.project, sex="F")
    ...
    def test_initial_page_no_filter(self):
        ...
        self.assertEqual(len(response.context["project_mice"]), 15)
        self.assertEqual(response.context["project_mice"].paginator.count, 36)
    ...
    def test_count_pagination_stats_after_filter_applied(self):
        ...
        self.assertEqual(response.context["project_mice"].paginator.count, 18)

    def test_filter_applied_on_another_page(self):
        self.data["page"] = 2
        response = self.client.get(self.url, data=self.data)
        self.assertEqual(response.context["project_mice"].number, 2)
        self.assertEqual(len(response.context["project_mice"]), 3)
        self.assertEqual(response.context["project_mice"].paginator.count, 18)
