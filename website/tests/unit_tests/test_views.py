...
class AddCageViewTest(TestCase):
    ...
    def test_add_cage_get_with_authenticated_user(self):
        response = self.client.get(reverse("add_cage"))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "add_cage.html")

    # POST CageForm with valid data
    def test_add_cage_post_valid(self):
        data = {
            "box_no": "1-1",
            "status": "Empty",
            "mother": "TestMother",
    ...
    def test_add_cage_get_with_unauthenticated_user(self):
        self.client.logout()
        response = self.client.get(reverse("add_cage"))
        url = reverse("add_cage")
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, f"/accounts/login/?next={url}")
...
class AddMouseViewTest(TestCase):
    ...
    # Access add_mouse while logged in
    def test_add_mouse_get(self):
        url = reverse("add_mouse", args=[self.project.projectname])
    ...
    def test_add_mouse_post_invalid(self):
        url = reverse("add_mouse", args=[self.project.projectname])
        data = {
            "sex": "Invalid",
            "dob": "2022-01-01",
...
class EditMouseViewTest(TestCase):
    ...
    def test_edit_mouse_get(self):
        ...
        self.assertIsInstance(response.context["form"], MiceForm)
